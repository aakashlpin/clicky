...
| `DelegationLogSidebar.swift` | ~1300 | Right-edge floating log viewer for delegated agent runs. Coordinates multiple concurrent delegation sessions — every launched agent gets its own independent panel, log tail, and process-lifecycle watcher, stacked along the right edge. Each session has mac-style traffic lights: red closes the session, yellow minimizes it to a compact bar that can be clicked to restore, green is decorative. The panel uses a `NonKeyingBorderlessPanel` subclass (`canBecomeKey = false`) so clicking inside it never steals key-window state from the user's target app. The boot banner displays the isolated git worktree path so the user can find the agent's checkout. For Claude Code runtimes, the sidebar feeds the raw log tail through a `DelegationLogClaudeStreamJSONParser` that understands Claude Code's `--output-format stream-json` frames (session init, streamed text deltas, tool uses, tool results, final result) and turns them into human-readable lines with live text-delta streaming. A pending-buffer flush runs at process exit so the final `result` frame appears even if it lands without a trailing newline. After the process exits, the sidebar runs `git rev-list --count base..working` in the worktree off the main actor and warns the user if zero commits were made. |
| `CodexExecLauncher.swift` | ~765 | Shared launcher service for Flowee's supported local coding-agent CLIs. Resolves the base branch (prefers `main` → `master` → remote default → current), generates a fresh branch name under `flowee/`, creates an **isolated git worktree** under `~/Library/Application Support/Flowee/Worktrees/<flattened-branch>` via `git worktree add -b`, then launches the selected runtime with its working directory (and any `-C`/`--dir` flags) pointed at the worktree. Writes logs to disk and returns branch + worktree + PR context for the delegation sidebar. When the caller supplies a semantic branch-name hint, the launcher sanitizes it, prefixes it with `flowee/`, checks for local collisions, and retries with a `-2`/`-3`/... numeric suffix before falling back to the legacy workspace-slug + timestamp + UUID format. The agent's prompt explicitly instructs it to `git add -A && git commit` before exiting so pull requests aren't empty. |
